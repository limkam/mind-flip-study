...
<body><main><article><header><span class="brand">🎓 BILKEYS</span><span class="period">My Scorecard</span></header>{display_name_html}{course_html}<p class="subtitle">{esc(activity_summary)}</p><div class="grid">{grid_html}</div>{badges_html}{message_html}<footer><a href="{esc(app_url)}">Think you can beat my streak? Join me on Bilkeys <span aria-hidden="true">🚀</span></a></footer></article></main></body></html>'''
