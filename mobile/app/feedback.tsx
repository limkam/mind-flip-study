...
export default function FeedbackScreen() {
  ...
  return <Screen keyboard>{header}<KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
    <View style={[styles.supportHeader, { borderColor: colors.border, backgroundColor: colors.surface }]}><Text style={[styles.supportTitle, { color: colors.text }]}>MindFlip Support</Text><Text style={[styles.subtitle, { color: colors.muted }]}>Usually responds as soon as possible</Text></View>
    <ScrollView ref={scroll} style={styles.flex} contentContainerStyle={styles.messages} keyboardShouldPersistTaps="handled" accessibilityLiveRegion="polite">
      ...
      {!chat.isLoading && !chat.isError && !messages.length && <View style={styles.empty}><Text style={[styles.emptyTitle, { color: colors.text }]}>How can we help?</Text><Text style={[styles.emptyBody, { color: colors.muted }]}>Send MindFlip feedback, report a problem, or ask us a question.{"\n"}No previous messages.</Text></View>}
