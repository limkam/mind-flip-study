...
export default function WordScrambleGame({ cards, onRoundComplete }) {
  ...
  const rounds = cards.flatMap((c) => {
    const raw = c.back.split(".")[0].split(",")[0].trim();
    const word = raw
      .replace(/[^a-zA-Z0-9 ]/g, "")
      .slice(0, 20)
      .toUpperCase();
    // Always include a round for each flashcard; if it can't be scrambled we'll fallback
    ...
    return (
      <div className="max-w-md mx-auto text-center bg-card rounded-3xl border border-border p-10">
        <p className="text-muted-foreground">
          Need flashcards with answers of at least two different letters to play
          Word Scramble.
        </p>
        <Button
          type="button"
          className="mt-4"
          onClick={() =>
            finishGame({ playerScore: 0, computerScore: 0, totalRounds: 1 })
          }
        >
          Back to games
        </Button>
    ...
    return (
      ...
      >
        <div className="flex justify-center gap-10 my-5">
          ...
          <div>
            <p className="text-3xl font-heading font-bold text-red-500">
              {computerScore}
            </p>
  ...
  const compProgress =
    ((COMPUTER_DELAY - compCountdown) / COMPUTER_DELAY) * 100;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-5">
        <Timer
          className={`w-4 h-4 ${timeLeft <= 5 ? "text-red-500" : "text-muted-foreground"}`}
        />
        ...
        <span
          className={`text-sm font-bold ${timeLeft <= 5 ? "text-red-500" : "text-muted-foreground"}`}
        >
          {timeLeft}s
        </span>
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-6">
        ...
        <button
          onClick={reshuffle}
          className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors ml-2"
        >
          ...
          >
            {roundResult === "player" ? (
              <>
                ...
                <p className="font-bold text-green-500 font-heading text-lg">
                  You solved it first! 🎉
                </p>
              </>
            ) : roundResult === "wrong" ? (
              <>
                ...
                <p className="font-bold text-red-500 font-heading text-lg">
                  Wrong answer!
                </p>
              </>
            ) : (
              <>
                ...
                <p className="font-bold text-red-500 font-heading text-lg">
                  Computer solved it first!
                </p>
              </>
            )}
            <p className="text-sm text-muted-foreground mt-1">
              Answer:{" "}
              <span className="font-bold text-foreground">{round.answer}</span>
            </p>
