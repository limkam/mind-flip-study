  ...
  if (gameOver) {
    return (
      <GameResultScreen
        icon={
          <div
            className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center ${winner === "player" ? "bg-green-500/10" : "bg-red-500/10"}`}
          >
            <Trophy
              className={`w-10 h-10 ${winner === "player" ? "text-green-500" : "text-red-500"}`}
            />
          </div>
        }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <p className="text-center text-sm text-muted-foreground mb-4">
        {answeredCount} of {questions.length} answered · Question{" "}
        {currentIdx + 1}
      </p>
        ...
        <div className="flex items-center gap-2 mb-3">
          <motion.span
            animate={
              ...
              selected !== questions[currentIdx]?.correct
                ? { x: [0, 8, 0, 8, 0] }
                : showResult && selected === questions[currentIdx]?.correct
                  ? {}
                  : {}
            }
          ...
          {showResult && (
            ...
            >
              {selected === questions[currentIdx]?.correct
                ? "💪 You pulled right!"
                : selected === "__timeout__"
                  ? "⏰ Time's up — computer pulls!"
                  : "❌ Wrong — computer pulls!"}
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        <Timer
          className={`w-4 h-4 ${timeLeft <= 3 ? "text-red-500" : "text-muted-foreground"}`}
        />
        ...
        <span
          className={`text-sm font-bold w-6 text-right ${timeLeft <= 3 ? "text-red-500" : "text-muted-foreground"}`}
        >
          {timeLeft}
        </span>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-border p-6 mb-5 shadow-sm">
        <p className="font-heading font-semibold text-lg leading-relaxed text-center">
          {q.question}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {q.options.map((option, i) => {
          ...
          if (showResult && isSelected && !isCorrect)
            cls = "border-red-500 bg-red-500/5";
