    ...
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <section className="flex min-h-[620px] flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4 sm:p-6" aria-live="polite">
          ...
          {messages.map((item) => { const own = item.sender_type === 'user'; return <div key={item.id} className={`flex ${own ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[82%] ${own ? 'text-right' : ''}`}><div className="mb-1 text-xs font-medium text-muted-foreground">{own ? 'You' : 'Bilkeys Support'}</div><div className={`whitespace-pre-wrap break-words rounded-2xl px-4 py-2.5 text-left text-sm ${own ? 'rounded-br-sm bg-primary text-primary-foreground' : 'rounded-bl-sm bg-muted'}`}>{item.body}</div><time className="mt-1 block text-xs text-muted-foreground">{time(item.created_at)}</time></div></div>; })}
          <div ref={bottomRef} />
        </div>
