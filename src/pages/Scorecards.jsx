...
function ScorecardView({ card, displayName = "Bilkeys Learner" }) {
  if (!card) return null;
  ...
  return <section className="relative aspect-[4/4.5] w-full overflow-hidden bg-[linear-gradient(145deg,#4f46e5_0%,#7c3aed_54%,#c026d3_100%)] p-[7%] font-body text-white shadow-2xl" aria-label={`${periodLabel} scorecard`}>
    ...
    <div className="relative flex h-full flex-col">
      <div className="flex items-center justify-between gap-3">
        <p className="flex items-center gap-3 text-[clamp(.75rem,2.4vw,1.35rem)] font-black tracking-[.16em]"><span aria-hidden="true">🎓</span><span>BILKEYS</span></p>
      ...
      <div className="mt-auto border-t border-white/25 pt-[5%] text-center text-[clamp(.72rem,2.15vw,1.15rem)] font-bold">Think you can beat my streak? Join me on Bilkeys <span aria-hidden="true">🚀</span></div>
    </div>
  </section>;
}
...
export default function Scorecards() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6 pb-12"><header><h1 className="font-heading text-3xl font-bold">Performance scorecards</h1><p className="mt-1 text-sm text-muted-foreground">Automatically refreshed summaries of meaningful learning activity.</p></header>
    ...
    <div ref={previewRef} className="mx-auto w-full max-w-[800px]"><ScorecardView card={card} displayName={user?.full_name || "Bilkeys Learner"} /></div>
